...
import Debian.Debianize.Types.Atoms (changelog, compat, control, copyright, install, installDir, installInit, intermediateFiles, link, logrotateStanza, packageDescription, postInst, postRm, preInst, preRm, rulesHead, sourceFormat, warning, watch)
...
import qualified Debian.Debianize.Types.BinaryDebDescription as B (architecture, binaryPriority, binarySection, breaks, builtUsing, conflicts, depends, description, essential, package, preDepends, provides, recommends, relations, replaces, suggests)
import Debian.Debianize.Types.CopyrightDescription (readCopyrightDescription)
import qualified Debian.Debianize.Types.SourceDebDescription as S (binaryPackages, buildConflicts, buildConflictsIndep, buildDepends, buildDependsIndep, dmUploadAllowed, homepage, newSourceDebDescription', priority, section, SourceDebDescription, standardsVersion, uploaders, vcsFields, VersionControlSpec(VCSArch, VCSBrowser, VCSBzr, VCSCvs, VCSDarcs, VCSGit, VCSHg, VCSMtn, VCSSvn), XField(XField), xFields)
