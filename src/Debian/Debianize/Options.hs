...
options =
    [ Option "" ["executable"] (ReqArg (\ path -> executableOption path (\ bin e -> doExecutable bin e)) "SOURCEPATH or SOURCEPATH:DESTDIR")
      ...
      Option "" ["default-package"] (ReqArg (\ name -> A.utilsPackageNameBase ~= Just name) "NAME")
      ...
      Option "" ["disable-haddock"] (NoArg (A.noDocumentationLibrary ~= True))
             (unlines [ "Don't generate API documentation packages, usually named"
                      ...
                      , "haddock bug."]),
      Option "" ["missing-dependency"] (ReqArg (\ name -> A.missingDependencies += (BinPkgName name)) "DEB")
             (unlines [ "This is the counterpart to --disable-haddock.  It prevents a package"
                      ...
                      , "build dependency."]),
      Option "" ["debian-name-base"] (ReqArg (\ name -> A.overrideDebianNameBase ~= (Just (DebBase name))) "NAME")
      ...
      Option "" ["source-package-name"] (ReqArg (\ name -> A.sourcePackageName ~= (Just (SrcPkgName name))) "NAME")
      ...
      Option "" ["disable-library-profiling"] (NoArg (A.noProfilingLibrary ~= True))
      ...
      Option "" ["maintainer"] (ReqArg (\ s -> either (error ("Invalid maintainer string: " ++ show s)) ((A.maintainerOption ~=) . Just) (parseMaintainer s)) "Maintainer Name <email addr>")
             (unlines [ "Supply a value for the Maintainer field.  Final value is computed from several inputs."]),
      Option "" ["uploader"] (ReqArg (\ s -> either (error ("Invalid uploader string: " ++ show s)) (\ x -> A.uploadersOption %= (\ l -> l ++ [x])) (parseMaintainer s)) "Uploader Name <email addr>")
      ...
      Option "" ["dev-dep"] (ReqArg (\ name -> A.extraDevDeps %= (++ [[Rel (BinPkgName name) Nothing Nothing]])) "Debian binary package name")
      ...
      Option "" ["map-dep"] (ReqArg (\ pair -> case break (== '=') pair of
                                                 (cab, (_ : deb)) -> A.extraLibMap +++= (cab, rels deb)
      ...
      Option "" ["deb-version"] (ReqArg (\ version -> A.debVersion ~= Just (parseDebianVersion version)) "VERSION")
             "Specify the version number for the debian package.  This will pin the version and should be considered dangerous.",
      Option "" ["revision"] (ReqArg (\ rev -> A.revision ~= Just rev) "REVISION")
      ...
      Option "" ["exec-map"] (ReqArg (\ s -> case break (== '=') s of
                                               (cab, (_ : deb)) -> A.execMap ++= (cab, rels deb)
      ...
      Option "" ["omit-prof-version-deps"] (NoArg (A.omitProfVersionDeps ~= True))
             "Do not put the version dependencies on the prof packages that we put on the dev packages.",
      Option "" ["omit-lt-deps"] (NoArg (A.omitLTDeps ~= True))
      ...
      Option "" ["official"] (NoArg (A.official ~= True))
             "This packaging is created of the official Debian Haskell Group",
      Option "" ["builddir"] (ReqArg (\ s -> A.buildDir ~= Just (s </> "build")) "PATH")
...
executableOption :: String -> (BinPkgName -> A.InstallFile -> a) -> a
executableOption arg f =
    case span (/= ':') arg of
      (sp, md) ->
          let (sd, name) = splitFileName sp in
          f (BinPkgName name)
            (A.InstallFile { A.execName = name
                         , A.destName = name
                         , A.sourceDir = case sd of "./" -> Nothing; _ -> Just sd
                         , A.destDir = case md of (':' : dd) -> Just dd; _ -> Nothing })
