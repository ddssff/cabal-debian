...
import Debian.Debianize.DebInfo (DebInfo, flags)
...
import Debian.Debianize.Types (buildDepends, buildDependsIndep, buildDir, conflicts, debVersion, depends, epochMap, execMap, extraDevDeps, extraLibMap, maintainerOption, missingDependencies, noDocumentationLibrary, noProfilingLibrary, official, omitLTDeps, omitProfVersionDeps, overrideDebianNameBase, provides, recommends, replaces, revision, sourceFormat, sourcePackageName, sourceSection, standardsVersion, suggests, uploadersOption, utilsPackageNameBase)
import Debian.Debianize.Types.Atoms (Atoms, debInfo, InstallFile(..))
