...
finalizeDebianization =
    do date <- liftIO getCurrentLocalRFC822Time
       ...
       vb <- access (A.debInfo . D.flags . verbosity)
...
finalizeDebianization' date debhelperCompat =
    do -- In reality, hcs must be a singleton or many things won't work.  But some day...
       ...
       access (A.debInfo . D.executable) >>= List.mapM_ (cabalExecBinaryPackage . fst) . Map.toList
       access (A.debInfo . D.backups) >>= List.mapM_ (cabalExecBinaryPackage . fst) . Map.toList
       access (A.debInfo . D.serverInfo) >>= List.mapM_ (cabalExecBinaryPackage . fst) . Map.toList
       access (A.debInfo . D.website) >>= List.mapM_ (cabalExecBinaryPackage . fst) . Map.toList
       ...
       debs <- access (A.debInfo . D.control . S.binaryPackages) >>= return . List.map (getL B.package)
       putBuildDeps debs pkgDesc
...
finalizeMaintainer = do
  o <- access (A.debInfo . D.official)
  currentUser <- liftIO getCurrentDebianUser
  pkgDesc <- access A.packageDescription
  maintainerOption <- access (A.debInfo . D.maintainerOption)
  uploadersOption <- access (A.debInfo . D.uploadersOption)
  let cabalAuthorString = takeWhile (\ c -> c /= ',' && c /= '\n') (Cabal.author pkgDesc)
      ...
      cabalMaintainerString'' = cabalAuthorString <> " " <> cabalMaintainerString
  changelogSignature <-
      do log <- access (A.debInfo . D.changelog)
         case log of
  ...
  case o of
    ...
    False -> do
      ...
      x <- access (A.debInfo . D.control . S.maintainer)
      when (isNothing x) 
...
finalizeControl =
    do finalizeMaintainer
       Just src <- access (A.debInfo . D.sourcePackageName)
...
finalizeChangelog date =
    do finalizeMaintainer
       ver <- debianVersion
       src <- access (A.debInfo . D.sourcePackageName)
       Just debianMaintainer <- access (A.debInfo . D.control . S.maintainer)
       ...
       let msg = pack ("Debianization generated by cabal-debian-" ++ showVersion version ++ " (" ++ show prog ++ ")")
...
addExtraLibDependencies hc =
    do pkgDesc <- access A.packageDescription
       ...
       libMap <- access (A.debInfo . D.extraLibMap)
       binNames <- List.map (getL B.package) <$> access (A.debInfo . D.control . S.binaryPackages)
...
checkOfficialSettings flavor =
    do o <- access (A.debInfo . D.official)
...
putBuildDeps :: (MonadIO m, Functor m) => [BinPkgName] -> PackageDescription -> CabalT m ()
putBuildDeps thisPackageDebs pkgDesc =
    do deps <- debianBuildDeps pkgDesc >>= return . filterRelations thisPackageDebs
       depsIndep <- debianBuildDepsIndep pkgDesc >>= return . filterRelations thisPackageDebs
...
binaryPackageRelations b typ =
    do edds <- access (A.debInfo . D.extraDevDeps)
...
librarySpecs pkgDesc hc =
    do let dev = isJust (Cabal.library pkgDesc)
       doc <- get >>= return . not . getL (A.debInfo . D.noDocumentationLibrary)
       prof <- get >>= return . not . getL (A.debInfo . D.noProfilingLibrary)
    ...
    do -- Files the cabal package expects to be installed
       ...
       installedExecMap <- Set.fold (\ x r ->
                                         case x of
                                           ...
                                           _ -> r) mempty <$> access (A.debInfo . D.atomSet) :: CabalT m (Map BinPkgName (Set String))

       -- The names of cabal executables that go into eponymous debs
       insExecPkg <- access (A.debInfo . D.executable) >>= return . Set.map ename . Set.fromList . elems
       ...
       access (A.debInfo . D.utilsPackageNameBase) >>= \ name ->
           case name of
...
expandAtoms =
    ...
    where
      ...
      expandApacheSites =
          do mp <- get >>= return . getL (A.debInfo . D.apacheSite)
      ...
      expandInstallCabalExecs builddir = do
        hc <- access (A.debInfo . D.flags . compilerFlavor)
        access (A.debInfo . D.atomSet) >>= Set.mapM_ (doAtom hc)
          where
...
finalizeRules =
    do DebBase b <- debianNameBase
       compiler <- access (A.debInfo . D.flags . compilerFlavor)
       ...
       flags <- (flagString . Set.toList) <$> access (A.debInfo . D.flags . cabalFlagAssignments)
